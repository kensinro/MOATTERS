...
r"""
MOATTERS
TCGA-LUAD Cross-Cancer Stage 2C — patient-level reconstruction,
...
Scope
-----
The LUAD-specific top-30 BP observables and r>=0.35 module architecture are
treated as fixed inputs from Stages 2A/2B. This stage:

1. Standardizes module scores across locked LUAD patients.
...
LUAD cohort in Stages 2A/2B. Therefore the repeated CV here audits only
...
STAGE2B = output_path(r"MOATTERS_LUAD_STAGE2B_NETWORK_MODULES")
...
OUT = output_path(r"MOATTERS_LUAD_STAGE2C_RECONSTRUCTION")
...
def survival_context(df: pd.DataFrame, score_cols: list[str]) -> pd.DataFrame:
    ...
    if CoxPHFitter is None:
        return pd.DataFrame([{"status": "lifelines_not_installed"}])
    ...
    for score_col in score_cols:
        ...
        rows.append({
            ...
            "p_value": float(cph.summary.loc["score_z", "p"]),
...
def main():
    ...
    with (OUT / "logs" / "luad_stage2c.log").open(
        "w", encoding="utf-8"
    ) as fh:
        log("Starting TCGA-LUAD Stage 2C reconstruction", fh)
        ...
        cv.to_csv(
            OUT / "tables" / "LUAD_centroid_only_repeated_CV_long.csv",
            index=False,
            encoding="utf-8-sig",
        )
        ...
        cv_split.to_csv(
            OUT / "tables" / "LUAD_centroid_only_repeated_CV_summary.csv",
            index=False,
            encoding="utf-8-sig",
        )
        ...
        patient = patient.join(
            cv_patient.drop(columns=["sample_index"]),
            how="left",
        )
        ...
        null.to_csv(
            OUT / "tables" / "LUAD_centroid_permutation_null.csv",
            index=False,
            encoding="utf-8-sig",
        )

        patient.to_csv(
            OUT / "tables" / "LUAD_patient_reconstruction_master.tsv",
            sep="\t",
            index=True,
        )
        module_z.to_csv(
            OUT / "matrices" / "LUAD_module_z_patients_x_modules.csv.gz",
            compression="gzip",
        )
        ...
        centroids.to_csv(
            OUT / "tables" / "LUAD_Early_Late_module_centroids.csv",
            index=False,
            encoding="utf-8-sig",
        )
        ...
        evaluation.to_csv(
            OUT / "tables" / "LUAD_stage_alignment_results.csv",
            index=False,
            encoding="utf-8-sig",
        )
        ...
        survival.to_csv(
            OUT / "tables" / "LUAD_survival_context_results.csv",
            index=False,
            encoding="utf-8-sig",
        )
        ...
        summary = {
            ...
            "stage": "TCGA_LUAD_STAGE2C_RECONSTRUCTION",
            "status": "PASS",
            ...
            "methodological_boundaries": [
                "LUAD-specific BP terms and modules were fixed from Stages 2A/2B.",
                "Repeated CV refitted only Early/Late centroids.",
                "This is not fully nested end-to-end validation because BP selection and module construction used the full LUAD cohort.",
            ...
            "next_step": (
                "Stage 2D: integrate LUAD cutoff/network sensitivity and "
                "reconstruction results into a reviewer-facing cross-cancer table."
            ),
        }
        with (OUT / "LUAD_STAGE2C_SUMMARY.json").open(
            "w", encoding="utf-8"
        ) as f:
            json.dump(summary, f, indent=2, ensure_ascii=False)

        (OUT / "README_LUAD_STAGE2C.txt").write_text(
            "MOATTERS — TCGA-LUAD Stage 2C\n\n"
            "Status: PASS\n"
            f"Patients: {len(samples)}; stage usable: {len(usable_samples)}\n"
            f"Modules: {module_z.shape[1]}\n"
            f"Apparent margin AUC: {apparent_margin['auc_directional']:.4f}\n"
            ...
            "from the full LUAD cohort.\n",
            encoding="utf-8",
        )

        log("LUAD Stage 2C completed: PASS", fh)
        log(
