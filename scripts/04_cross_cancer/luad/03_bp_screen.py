...
r"""
MOATTERS
TCGA-LUAD Cross-Cancer Stage 2A — GO-BP observation-readiness and
...
Primary contract
----------------
- Primary cohort: TCGA-LUAD primary tumours.
- Primary contrast: Stage I/II (Early) vs Stage III/IV (Late).
- Gene expression is standardized gene-wise across locked LUAD patients.
...
- No BRCA or KIRC BP term list or module architecture is transferred.
...
CLINICAL = (
    STAGE1 / "tables" /
    "LUAD_primary_clinical_harmonized.tsv"
)
GMT = data_path(r"GSEA/c5.go.bp.v2026.1.Hs.symbols.gmt")
OUT = output_path(r"MOATTERS_LUAD_STAGE2A_BP_SCREEN")
...
def main():
    ...
    with (OUT / "logs" / "luad_stage2a.log").open(
        "w", encoding="utf-8"
    ) as fh:
        log("Starting TCGA-LUAD Stage 2A BP screening", fh)
        ...
        results.to_csv(
            OUT / "tables" / "LUAD_GO_BP_stage_discriminability_all.csv",
            index=False,
            encoding="utf-8-sig",
        )
        ...
        selected.to_csv(
            OUT / "tables" / "LUAD_selected_BP_nominal_p_lt_0_05.csv",
            index=False,
            encoding="utf-8-sig",
        )

        pd.DataFrame(matched_gene_rows).to_csv(
            OUT / "tables" / "LUAD_K10_BP_matched_gene_manifest.csv",
            index=False,
            encoding="utf-8-sig",
        )
        ...
        if score_columns:
            ...
            bp_scores.loc[
                ...
                OUT / "matrices" /
                "LUAD_selected_BP_scores_terms_x_patients.csv.gz",
                compression="gzip",
            )
        ...
        readiness.to_csv(
            OUT / "tables" / "LUAD_observation_readiness_by_K.csv",
            index=False,
            encoding="utf-8-sig",
        )
        ...
        for d_cut in D_THRESHOLDS:
            d_rows.append({
                ...
                "n_K10_BP_selected": int(
                    pd.to_numeric(eligible["D"], errors="coerce")
                    .ge(d_cut).sum()
                ),
            })
        d_sensitivity = pd.DataFrame(d_rows)
        d_sensitivity.to_csv(
            OUT / "tables" / "LUAD_D_threshold_sensitivity_counts.csv",
            index=False,
            encoding="utf-8-sig",
        )
        ...
        summary = {
            ...
            "stage": "TCGA_LUAD_STAGE2A_BP_SCREEN",
            "status": status,
            ...
            "methodological_boundaries": [
                "LUAD received its own GO-BP screening.",
                "No BRCA or KIRC BP terms or modules were transferred.",
                "The nominal p<0.05 threshold is a reconstruction-oriented screen, not an independent validation claim for each BP.",
                "Network construction and module reconstruction will be performed only in the next stage.",
            ],
            "next_step": (
                "Stage 2B: construct the LUAD Late-stage BP-correlation network, "
                "audit r/p thresholds, and derive LUAD-specific modules."
            ),
        }
        with (OUT / "LUAD_STAGE2A_SUMMARY.json").open(
            "w", encoding="utf-8"
        ) as f:
            json.dump(summary, f, indent=2, ensure_ascii=False)

        (OUT / "README_LUAD_STAGE2A.txt").write_text(
            "MOATTERS — TCGA-LUAD Stage 2A\n\n"
            f"Status: {status}\n"
            ...
            "No BRCA/KIRC BP or module definitions were transferred.\n",
            encoding="utf-8",
        )

        log(f"LUAD Stage 2A completed: {status}", fh)
        log(
