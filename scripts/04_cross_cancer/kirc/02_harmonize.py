...
OUT_DIR = output_path(r"MOATTERS_KIRC_STAGE1")
...
def harmonize_clinical(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # UCSC Xena KIRC uses LIVING / DECEASED. Accept common synonyms as well.
    dead_mask = vital.str.contains(
        r"DECEASED|DEAD", na=False, regex=True
    )
    living_mask = vital.str.contains(
        r"LIVING|ALIVE", na=False, regex=True
    )
...
def main():
    ...
    with (OUT_DIR / "logs" / "kirc_stage1.log").open(
        ...
        locked["vital_status_clean"] = locked["vital_status"].astype("string").str.upper().str.strip()
        ...
        os_event_count = int(
            pd.to_numeric(clinical_locked["OS_event"], errors="coerce").eq(1).sum()
        )
        status = (
            "PASS"
