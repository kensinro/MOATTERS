...
r"""
MOATTERS
TCGA-KIRC Cross-Cancer Stage 2B — top-30 BP lock, late-state network,
correlation-threshold sensitivity and KIRC-specific modules
...
-------------------------
1. Select the 30 most stage-discriminative KIRC GO-BP observables among
   K>=10 observation-ready terms, ranked by D=-log10(p).
2. Use only Late-stage KIRC patients to estimate the BP-BP Pearson network.
3. Primary network threshold: |r| >= 0.35.
4. Sensitivity thresholds: |r| >= 0.25, 0.35, 0.45.
5. Derive KIRC-specific communities by greedy modularity optimization.
6. Isolated nodes remain singleton modules.
7. No BRCA BP terms, edges, centroids, modules or patient labels are reused.

This stage derives the KIRC representation architecture only. Patient-level
...
STAGE2A = output_path(r"MOATTERS_KIRC_STAGE2A_BP_SCREEN")
CLINICAL = STAGE1 / "tables" / "KIRC_primary_clinical_harmonized.tsv"
ALL_RESULTS = (
    STAGE2A / "tables" / "KIRC_GO_BP_stage_discriminability_all.csv"
)
BP_SCORES = (
    STAGE2A / "matrices" /
    "KIRC_K10_observation_ready_BP_scores_terms_x_patients.csv.gz"
)
OUT = output_path(r"MOATTERS_KIRC_STAGE2B_NETWORK_MODULES")
...
def graph_and_modules(
    ...
    kept = edges[pd.to_numeric(edges["abs_r"], errors="coerce").ge(threshold)]
...
def main():
    ...
    with (OUT / "logs" / "kirc_stage2b.log").open(
        "w", encoding="utf-8"
    ) as fh:
        log("Starting TCGA-KIRC Stage 2B network/module derivation", fh)
        ...
        locked["KIRC_rank_by_D"] = np.arange(1, TOP_N + 1)
        locked.to_csv(
            OUT / "tables" / "KIRC_top30_BP_lock.csv",
        ...
        late_matrix.to_csv(
            OUT / "matrices" / "KIRC_top30_BP_scores_late_patients.csv.gz",
            compression="gzip"
        )

        edges = build_edge_table(late_matrix)
        edges.to_csv(
            OUT / "tables" / "KIRC_top30_late_all_pairwise_correlations.csv",
            index=False, encoding="utf-8-sig"
        )
        ...
        for threshold in R_THRESHOLDS:
            ...
            kept.to_csv(
                OUT / "tables" /
                f"KIRC_top30_edges_abs_r_ge_{threshold:.2f}.csv",
                index=False, encoding="utf-8-sig"
            )
        ...
        sensitivity.to_csv(
            OUT / "tables" /
            "KIRC_network_threshold_sensitivity_summary.csv",
            index=False, encoding="utf-8-sig"
        )
        assignments = pd.DataFrame(assignment_rows)
        assignments.to_csv(
            OUT / "tables" /
            "KIRC_module_assignments_all_r_thresholds.csv",
            index=False, encoding="utf-8-sig"
        )
        ...
        # Module score = mean BP score across member terms, all locked patients.
        ...
        summary = {
            ...
            "stage": "TCGA_KIRC_STAGE2B_NETWORK_MODULES",
            "status": status,
            "top_N_BP": TOP_N,
            "selection_rule": (
                "Top 30 K>=10 KIRC GO-BP terms ranked by D=-log10(p) "
            ...
            "methodological_boundaries": [
                "The top-30 BP lock is KIRC-specific.",
                "The BP network was estimated only in Late-stage KIRC patients.",
                "The primary edge threshold was |r|>=0.35.",
                "Threshold sensitivity was assessed at |r|>=0.25, 0.35 and 0.45.",
                "BRCA BP terms, modules and centroids were not transferred.",
                "Patient reconstruction has not yet been evaluated.",
            ],
            "next_step": (
                "Stage 2C: derive Early/Late centroids in KIRC module space, "
                "reconstruct patient-level molecular-functional states, and "
                "audit internal stability/null behavior."
            ),
        }
        with (OUT / "KIRC_STAGE2B_SUMMARY.json").open(
            "w", encoding="utf-8"
        ) as f:
            json.dump(summary, f, indent=2, ensure_ascii=False)

        (OUT / "README_KIRC_STAGE2B.txt").write_text(
            "MOATTERS — TCGA-KIRC Stage 2B\n\n"
            f"Status: {status}\n"
            ...
            "The architecture is KIRC-specific; no BRCA module transfer occurred.\n",
            encoding="utf-8",
        )

        log(f"KIRC Stage 2B completed: {status}", fh)
        log(
