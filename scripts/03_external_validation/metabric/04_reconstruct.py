...
r"""
MOATTERS
METABRIC Stage 2C — locked patient-level external reconstruction

Primary analysis
----------------
- External cohort: METABRIC, n=1,980
...
Important boundary
------------------
No METABRIC endpoint is used to select BP terms, form modules, estimate risk
direction, or estimate early/late centroids. Gene and module values are
standardized within METABRIC using the historical implementation, after which
the locked TCGA-BRCA centroids and direction are applied.

Output
------
D:\MOATTERS-Output\MOATTERS_METABRIC_STAGE2C_RECONSTRUCTION
"""
...
STAGE2B = output_path(r"MOATTERS_METABRIC_STAGE2B_LOCK_COVERAGE_V1_1")
OUT = output_path(r"MOATTERS_METABRIC_STAGE2C_RECONSTRUCTION")
...
EXPRESSION_CANDIDATES = [
    STAGE1 / "matrices" / "METABRIC_primary_expression_genes_x_samples.tsv.gz",
    STAGE1 / "matrices" / "METABRIC_primary_expression_genes_x_samples.tsv",
]
CLINICAL_CANDIDATES = [
    STAGE1 / "matrices" / "METABRIC_clinical_harmonized.tsv",
    STAGE1 / "tables" / "METABRIC_clinical_harmonized.tsv",
]
...
def main() -> None:
    ...
    with (OUT / "logs" / "stage2c_reconstruction.log").open(
        ...
        log("Starting locked METABRIC patient-level reconstruction", fh)
        ...
        log(f"METABRIC expression loaded: {expression.shape[0]} genes x {expression.shape[1]} samples", fh)
        ...
        log("Computing row-wise METABRIC gene z-scores", fh)
        ...
        for k in K_VALUES:
            ...
            bp_readiness.to_csv(
                OUT / "audit" / f"METABRIC_BP_readiness_K{k}.csv",
                index=False, encoding="utf-8-sig"
            )
            module_coverage.to_csv(
                OUT / "audit" / f"METABRIC_module_BP_retention_K{k}.csv",
                index=False, encoding="utf-8-sig"
            )
            module_raw.to_csv(
                OUT / "matrices" / f"METABRIC_module_scores_raw_K{k}.csv"
            )
            module_z.to_csv(
                OUT / "matrices" / f"METABRIC_module_scores_z_K{k}.csv"
            )
            metrics.to_csv(
                OUT / "tables" / f"METABRIC_patient_reconstruction_K{k}.csv",
                index=False, encoding="utf-8-sig"
            )
        ...
        primary.to_csv(
            OUT / "METABRIC_patient_reconstruction_PRIMARY_K10.csv",
            index=False, encoding="utf-8-sig"
        )
        ...
        stability.to_csv(
            OUT / "tables" / "METABRIC_cross_K_patient_stability.csv",
            index=False, encoding="utf-8-sig"
        )
        pd.concat(class_tables, ignore_index=True).to_csv(
            OUT / "tables" / "METABRIC_state_class_counts_by_K.csv",
            index=False, encoding="utf-8-sig"
        )
        ...
        pd.DataFrame(module_rows).to_csv(
            OUT / "tables" / "METABRIC_cross_K_module_score_stability.csv",
            index=False, encoding="utf-8-sig"
        )

        # Locked contract and provenance.
        contract = {
            "external_cohort": "METABRIC",
            ...
            "gene_standardization": "row-wise z-score across METABRIC patients",
            ...
            "module_standardization": "row-wise z-score across METABRIC patients",
        ...
        with (OUT / "METABRIC_STAGE2C_LOCKED_CONTRACT.json").open(
            "w", encoding="utf-8"
        ) as f:
        ...
        summary = {
            ...
            "stage": "METABRIC_STAGE2C_LOCKED_EXTERNAL_RECONSTRUCTION",
            "status": "PASS",
            "primary_K": PRIMARY_K,
        ...
        (OUT / "README_STAGE2C.txt").write_text(
            ...
            "No METABRIC endpoint was used to define or fit the reconstruction.\n",
            encoding="utf-8",
        )
